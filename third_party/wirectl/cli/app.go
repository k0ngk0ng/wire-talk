...
func (a App) Run(ctx context.Context, args []string) error {
	...
	if a.PluginPrefix != "" && commandName.MatchString(args[0]) {
		...
		if self, selfErr := os.Executable(); selfErr == nil {
			...
			if st, statErr := os.Stat(sibling); statErr == nil && st.Mode().IsRegular() && (runtime.GOOS == "windows" || st.Mode()&0111 != 0) {
...
func plugins(prefix string) []string {
	...
	for _, dir := range dirs {
		...
		for _, entry := range entries {
			...
			if name == filename || !commandName.MatchString(name) {
				continue
			}
			if st, err := os.Stat(filepath.Join(dir, entry.Name())); err == nil && st.Mode().IsRegular() && (runtime.GOOS == "windows" || st.Mode()&0111 != 0) {
				found[name] = true
			}
		}
	}
